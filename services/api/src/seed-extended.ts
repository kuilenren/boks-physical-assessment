...
const FAMILY_DEFS: Array<{ id: string; display_name: string; children: SeedChild[] }> = [
  {
    id: "family-primary-low-001",
    display_name: "张家（小学低段）",
    children: [
      { id: "child-001", display_name: "张一", birth_date: "2019-09-01", sex_code: "male", school_stage: "primary", grade_code: "2" },
    ],
  },
  {
    ...
    children: [
      { id: "child-002", display_name: "李二", birth_date: "2014-03-12", sex_code: "female", school_stage: "primary", grade_code: "5" },
    ],
  },
  {
    ...
    display_name: "王家（初中）",
    children: [
      { id: "child-003", display_name: "王三", birth_date: "2010-07-22", sex_code: "male", school_stage: "junior_high", grade_code: "8" },
    ],
  },
  {
    ...
    display_name: "赵家（高中）",
    children: [
      { id: "child-004", display_name: "赵四", birth_date: "2007-11-05", sex_code: "female", school_stage: "senior_high", grade_code: "11" },
    ],
  },
  {
    ...
    display_name: "钱家（幼儿）",
    children: [
      { id: "child-005", display_name: "钱五", birth_date: "2021-04-18", sex_code: "female", school_stage: "preschool", grade_code: "kindergarten-2" },
    ],
  },
  {
    id: "family-multi-006",
    display_name: "孙家（多子女）",
    children: [
      { id: "child-006", display_name: "孙六", birth_date: "2013-08-30", sex_code: "male", school_stage: "primary", grade_code: "4" },
      { id: "child-007", display_name: "孙七", birth_date: "2011-02-14", sex_code: "female", school_stage: "junior_high", grade_code: "7" },
    ],
  },
];

function runForStage(grade: string, sex: "male" | "female", stage: SeedChild["school_stage"]): Array<{ indicator_code: string; raw_value: string }> {
  if (stage === "preschool") return [];
  if (stage === "primary") {
    const g = parseInt(grade);
    const age = 6 + g;
    return [
      { indicator_code: "height", raw_value: String(110 + age * 5 + (sex === "male" ? 2 : 0)) },
      { indicator_code: "weight", raw_value: String(19 + age * 2.2 + (sex === "male" ? 0.8 : 0)) },
      { indicator_code: "run_50m", raw_value: String((sex === "male" ? 10.0 : 10.6) + g * 0.3) },
      { indicator_code: "sit_reach", raw_value: String((sex === "male" ? 8 : 12) + g) },
      { indicator_code: "rope_1min", raw_value: String((sex === "male" ? 80 : 75) + g * 5) },
      { indicator_code: "lung_capacity", raw_value: String(800 + g * 100) },
    ];
  }
  if (stage === "junior_high") {
    return [
      { indicator_code: "height", raw_value: String(sex === "male" ? 160 : 158) },
      { indicator_code: "weight", raw_value: String(sex === "male" ? 50 : 47) },
      { indicator_code: "run_50m", raw_value: String(sex === "male" ? 8.0 : 8.6) },
      { indicator_code: "sit_reach", raw_value: String(sex === "male" ? 8 : 11) },
      { indicator_code: "rope_1min", raw_value: String(sex === "male" ? 160 : 155) },
      { indicator_code: "lung_capacity", raw_value: "3000" },
      { indicator_code: "standing_jump", raw_value: String(sex === "male" ? 200 : 175) },
      ...(sex === "male"
        ? [{ indicator_code: "pull_up", raw_value: "3" }]
        : [{ indicator_code: "sit_up", raw_value: "30" }]),
      { indicator_code: "endurance_run", raw_value: String(sex === "male" ? 270 : 250) },
    ];
  }
  return [
    { indicator_code: "height", raw_value: String(sex === "male" ? 173 : 162) },
    { indicator_code: "weight", raw_value: String(sex === "male" ? 62 : 53) },
    { indicator_code: "run_50m", raw_value: String(sex === "male" ? 7.3 : 8.0) },
    { indicator_code: "sit_reach", raw_value: String(sex === "male" ? 10 : 13) },
    { indicator_code: "rope_1min", raw_value: String(sex === "male" ? 180 : 170) },
    { indicator_code: "lung_capacity", raw_value: "4000" },
    { indicator_code: "standing_jump", raw_value: String(sex === "male" ? 235 : 190) },
    ...(sex === "male"
      ? [{ indicator_code: "pull_up", raw_value: "6" }]
      : [{ indicator_code: "sit_up", raw_value: "38" }]),
    { indicator_code: "endurance_run", raw_value: String(sex === "male" ? 240 : 230) },
  ];
}
...
export function seedExtendedFamilies(): {
  ...
  const counts = { families: 0, children: 0, assessments: 0, trainings: 0, postures: 0, conversations: 0 };
  ...
  for (const f of FAMILY_DEFS) {
    ...
    if (exChildren.filter((c: { family_id: string }) => c.family_id === f.id).length > 0 && Object.keys(exSessions).length > 0) {
      continue;
    }
    ...
    updateFamilyStore(f.id, (family) => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (family as any).metadata = { ...((family as any).metadata ?? {}), seeded: true, seeded_at: NOW };
      ...
      for (const sc of f.children) {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const exists = (family as any).children.find((c: { id: string }) => c.id === sc.id);
        if (!exists) {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ...
        if (values.length > 0) {
          ...
          if (!(family as any).assessmentSessions) (family as any).assessmentSessions = {};
        ...
        if (!(family as any).trainingPlans[planId] && sc.school_stage !== "preschool") {
          (family as any).trainingPlans[planId] = {
            ...
            status: "active",
            duration_days: 84,
            weekly_template: ["balance", "core", "jump", "endurance", "stretch", "rest", "rest"],
            created_at: NOW,
            updated_at: NOW,
          };
          counts.trainings++;
          ...
          for (let day = 1; day <= 14; day++) {
            ...
            (family as any).checkIns[ciId] = {
              ...
              child_id: sc.id,
              day,
              status: day % 7 === 0 ? "skipped" : day % 3 === 0 ? "partial" : "done",
              created_at: NOW,
            };
          }
        }
        ...
        // 体态（部分家庭）
        if (["family-primary-low-001", "family-junior-003", "family-multi-006"].includes(f.id)) {
          const psId = `ps-${sc.id}-seed`;
          if (!(family as any).postureSessions) (family as any).postureSessions = {};
          if (!(family as any).postureSessions[psId]) {
            ...
            if (!(family as any).postureAssets) (family as any).postureAssets = {};
            ...
            if (!(family as any).postureReports) (family as any).postureReports = {};
            const prId = `pr-${psId}`;
        ...
        if (!(family as any).conversations[cvId] && sc.school_stage !== "preschool") {
          (family as any).conversations[cvId] = {
            ...
            messages: [
              ...
              },
              {
                ...
                citations: [
                  { source_id: "src-national-2014-standard", title: "国家学生体质健康标准（2014）", version: "2014修订" },
                  { source_id: "src-training-safety", title: "儿童体测训练安全", version: "1.0" },
                ],
                created_at: NOW,
              },
            ],
